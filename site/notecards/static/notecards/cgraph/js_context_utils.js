...
const M = Math;
const P = (x, y) => new utils.Point(x, y);
const B = (x, y, width, height) => new utils.Bounds(x, y, width, height);


/*
 * range(start, stop [, step] [, func])
 *
 * Returns an array containing values in the half open interval
 * [start, stop). If the optional parameter func is provided
 * then func is called for each value in the range and its return
 * value is put in its place.
 *
 * start: number
 *        Start of the interval. The interval includes this value.
 *
 * stop:  number
 *        End of the interval. The interval does not normally include
 *        this value.
 *
 ...
 *          1. a value in the range defined by the previous parameters.
 *          2. the step value
 *        The return value of this function is used as the value
 *        for each corresponding item in the array. This function
 *        is called before each insertion of the value in the array.
 */
const range = function (...args)
{
    ...
    {
        let start = parseFloat(args[0]);
        let stop = parseFloat(args[1]);
        let step = (typeof args[2] == 'number') ? parseFloat(args[2]) : 1;
        let func = null;

        if (typeof args[2] == 'function') func = args[2];
        else if (typeof args[3] == 'function') func = args[3];

        if ((start < stop) && (step > 0))
        {
            while (start < stop)
            {
                result.push(func ? func(start, step) : start);
                start += step;
            }
        }
...
function runCode(code)
{
    ...
    let returnValue = func(returnContext);

    if (returnContext.r !== '') return returnContext.r;
